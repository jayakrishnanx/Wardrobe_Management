...
class OutfitFeedback(models.Model):
    ...
    rating = models.FloatField(
        help_text="User rating from 1.0 to 5.0",
        validators=[MinValueValidator(1.0), MaxValueValidator(5.0)]
    )
    feedback_text = models.TextField(blank=True, null=True)
    ai_review = models.TextField(blank=True, null=True, help_text="AI's reasoning for score adjustment")
    is_read = models.BooleanField(default=False, help_text="Checked by admin")
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Feedback by {self.user.username} on Outfit {self.recommendation.id}"
